                ...
                for type_bounds in type_bounds_list:
                    ...
                    while not done:
                        ...
                        if not model_created:
                            ...
                            verif_model,mdenv_count = env_cut_verif_model_lp_sol(n_layers,n_neurons,activation,filtered_params,bounds,verif_model,all_vars,lp_sol_file)
                        ...
                        solution = np.array([verif_model.getVal(all_vars['h{},{}'.format(-1,i)]) for i in range(len(image_list))])
                        if prev_sol != None:
                            distance = np.linalg.norm(solution-prev_sol)
                            if distance <= 1E-05:
                                done = True
                            else:
                                prev_sol = solution
                    ...
                    print('gap:',gap)
                ## Nombre del archivo xlsx donde se guardan los resultados de los experimentos
                file_name = calculate_verif_file_name('env_cut_rounds',activation,real_output,target_output,root_node_only,True,initial_sol,type_bounds)
