...
for activation in activation_list:
    ## Archivo xlsx
    file_name = 'bounds_calculation/{}_{}rounds_{}minutes_10neurons.xlsx'.format(activation,max_rounds,minutes)
    header = ['L','n']
    for k in type_cuts_list:
        for l in range(max(layer_list)):
            header += ['{}:time'.format(l),'{}:rounds'.format(l),'{}:cuts'.format(l),'{}:lb gap [%]'.format(l),'{}:ub gap [%]'.format(l+2)]
    ...
    for n_layers in layer_list:
        ...
        for n_neurons in neuron_list:
            if (activation == 'selu' and ((n_layers == 5 and n_neurons == 10) or (n_layers == 5 and n_neurons == 60))) or (activation == 'b' and not ((n_layers == 5 and n_neurons == 7) or (n_layers == 6 and n_neurons == 5))):
                continue
            else:
                ...
                for tol_distance in tols_list_range:
                    ...
                    for type_cut in type_cuts_list:
                        ...
                        for l in range(n_layers):
                            ...
                            for i in range(layer_neurons):
                                ...
                                for q in range(2):
                                    ...
                                    print(' Sentido:',sense)
                                    while reason is None and round_count < max_rounds and neuron_time <= minutes*60:
                                        print('\n === Ronda {}, {},{} === \n'.format(round_count+1,l,i))
                                        ## Tiempo de la ronda
                                        ...
                                        if l < 1 or type_cut == 'R_H':
                                            ...
                                            if prev_obj is None:
                                                ...
                                                if np.abs(obj-prev_obj) < 1E-05:
                                                    reason = 'La solucion converge'
                                                else:
                                                    prev_obj = obj
                                            ## Se obtiene la solucion
                                            lp_sol = get_bounds_model_lpsol(l,bounds_model,all_vars,params,activation)
                                            ## Se libera el modelo
                                            bounds_model.freeTransform()
                                            if add_verif_bounds:
                                                accepted = add_bounds_vars_by_image(bounds_model,sol_dict)
                                            ...
                                            if reason is None and mdenv_count == 0:
                        ...
                        bounds_file = 'nn_bounds/{}_bounds_L{}_n{}_{}_cut_{}rounds_{}minutes.txt'.format(activation,n_layers,n_neurons,type_cut,max_rounds,minutes)
